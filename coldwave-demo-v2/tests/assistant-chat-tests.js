...
module.exports = async function runAssistantChatTests() {
  ...
  class FakeDocument {
    constructor() {
      ...
      const suggestions = [
        ...
        ["What does the recovery-censored warning mean?", "section"],
      ...
      suggestions.forEach(([question, requirement]) => {
  ...
  test("composer and suggestions are visible only in backend-agent mode", () => {
    ...
    includes(elements.assistantDisclaimer.textContent, "Local Qwen");
  });
  ...
  test("pending state blocks duplicate submission and exposes accessible cancellation", async () => {
    ...
    assert(elements.assistantChatMessages.getAttribute("aria-busy") === "true");
    assert(!elements.assistantCancel.hidden && !elements.assistantCancel.disabled);
    assert(elements.assistantInput.disabled);
    equal(elements.assistantChatStatus.textContent, "Local Qwen is working…");
  ...
  test("Cancel aborts the request, adds no assistant history, and returns focus", async () => {
    ...
    includes(elements.assistantChatMessages.textContent, "request cancelled");
  ...
  test("long valid answers are bounded before entering the next request history", async () => {
    ...
    includes(elements.assistantChatMessages.textContent, longAnswer);
  ...
  test("successful history resets without an automatic request on context changes", async () => {
    ...
    includes(elements.assistantChatMessages.textContent, "No question was sent");
  ...
  test("section changes clear history, cancel pending work, and ignore stale responses", async () => {
    ...
    includes(elements.assistantChatMessages.textContent, "CS_257");
    includes(elements.assistantChatMessages.textContent, "No question was sent");
    gate.resolve(responseFixture({ answer: "STALE RESPONSE" }));
    assert(await pending === false);
    excludes(elements.assistantChatMessages.textContent, "STALE RESPONSE");
  });
  ...
  test("metric changes clear history and cancel a pending metric-context request", async () => {
    ...
    equal(controller.getState().historyLength, 0);
    includes(elements.assistantChatMessages.textContent, "event_rei");
    gate.resolve(responseFixture({ answer: "STALE METRIC RESPONSE" }));
    assert(await pending === false);
    excludes(elements.assistantChatMessages.textContent, "STALE METRIC RESPONSE");
  });
  ...
  test("all seven reviewed response statuses have explicit presentation", async () => {
    ...
    for (const status of statuses) {
      ...
      includes(
        elements.assistantChatMessages.textContent.toLowerCase(),
        status.replaceAll("_", " "),
        status
      );
      if (status === "clarification_required") {
        includes(elements.assistantChatMessages.textContent, "Which control section");
        includes(elements.assistantChatMessages.textContent, `Test ${status}.`);
      }
      if (status === "invalid_model_response") {
        excludes(elements.assistantChatMessages.textContent, "REJECTED MODEL TEXT");
        includes(elements.assistantChatMessages.textContent, "could not be safely accepted");
      }
      if (status === "model_unavailable") {
        includes(elements.assistantChatMessages.textContent, "local Qwen model is unavailable");
        includes(elements.assistantChatMessages.textContent, "Local assistant status");
      }
    }
  });
  ...
  test("timeout and client cancellation failures are sanitized without a fake fallback", async () => {
    for (const code of ["request_timeout", "request_cancelled"]) {
      ...
      includes(elements.assistantChatMessages.textContent, code === "request_timeout"
        ? "did not complete"
        : "was cancelled");
      excludes(elements.assistantChatMessages.textContent, "80000");
      excludes(elements.assistantChatMessages.textContent, "local template");
      equal(controller.getState().historyLength, 0);
    }
  });
  ...
  test("structured response renders inert text, warnings, tools, evidence, limitations, and metadata", async () => {
    ...
    const text = elements.assistantChatMessages.textContent;
    includes(text, scriptLike);
    includes(text, "Local Qwen3 8B + deterministic tools");
    ...
    const descendants = allDescendants(elements.assistantChatMessages);
  ...
  return `${tests.length} Phase 3G Assistant chat interaction tests passed.`;
};
