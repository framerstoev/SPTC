(async function runAssistantClientTests(global) {
  ...
  function metricFixture(name = "q_min") {
    const planning = name === "netrisk_lite";
    return {
      metric: {
        name,
        display_name: planning ? "Network REI" : "Minimum normalized Q",
      ...
      provenance: {
        source_label: "Jan. 2026 cold-wave processed control-section resilience release",
        source_field: planning ? "NETRISK_LITE" : "q_min",
        method_version: "data_driven_resilience_v0"
      },
      event_id: "coldwave_2026_01",
      data_release: "coldwave_2026_01_r1",
  ...
  function v3MetricDescriptor(metric, displayName, sourceMetric) {
    return {
      metric,
      display_name: displayName,
      source_metric: sourceMetric,
      unit: "dimensionless",
      higher_value_interpretation: "Higher values indicate more favorable context within this release."
    };
  }

  function v3Distribution(populationCount, availableCount, minimum = 0.1, median = 0.5, maximum = 0.9) {
    return {
      population_count: populationCount,
      available_count: availableCount,
      missing_count: populationCount - availableCount,
      minimum: availableCount === 0 ? null : minimum,
      median: availableCount === 0 ? null : median,
      maximum: availableCount === 0 ? null : maximum
    };
  }

  function v3SectionValue(position, value, overrides = {}) {
    return {
      position,
      metric_rank: overrides.metricRank || position,
      section_id: overrides.sectionId || `CS_${1000 + position}`,
      route: overrides.route || "IH_35_",
      county: overrides.county || "Dallas",
      value,
      observed_support: overrides.observedSupport !== false,
      detection_status: overrides.detectionStatus || "detected"
    };
  }

  function v3RankingFixture() {
    return {
      result_type: "section_ranking",
      metric: v3MetricDescriptor(
        "tier3_observed_resilience",
        "Tier 3 Observed Resilience",
        "observed_curve_resilience_score_v0"
      ),
      direction: "descending",
      county: null,
      limit_per_tail: 2,
      distribution: v3Distribution(10029, 3842),
      highest_sections: [v3SectionValue(1, 0.9), v3SectionValue(2, 0.8)],
      lowest_sections: [
        v3SectionValue(1, 0.1, { metricRank: 3842, sectionId: "CS_2001" }),
        v3SectionValue(2, 0.2, { metricRank: 3841, sectionId: "CS_2002" })
      ],
  ...
  function v3CountyFixture() {
    const descriptors = [
      v3MetricDescriptor("tier1_weather", "Tier 1 Weather", "weather_rei"),
      v3MetricDescriptor("tier2_network", "Tier 2 Network", "netrisk_lite"),
      v3MetricDescriptor("potential_resilience", "Potential Resilience", "potential_resilience_score"),
      v3MetricDescriptor("tier3_observed_resilience", "Tier 3 Observed Resilience", "observed_curve_resilience_score_v0")
    ];
    return {
      ...
      coverage_caveat: "Two sections have no observed support; missing support is not no disruption.",
      event_id: "coldwave_2026_01",
      data_release: "coldwave_2026_01_r1",
      method_version: "data_driven_resilience_v0"
    };
  ...
  function v3AlignmentFixture() {
    return {
      ...
      method_note: "Correlation is descriptive; agreement categories require a reviewed definition.",
      event_id: "coldwave_2026_01",
      data_release: "coldwave_2026_01_r1",
      method_version: "data_driven_resilience_v0"
    };
  ...
  function fakeWindow(href, overrides = {}) {
    ...
    return {
      SPTCV3Deployment: Object.freeze({
        backend_target: overrides.backendTarget || "local-loopback"
      }),
  ...
  function successfulFetch(calls, summaryPayload = summaryFixture()) {
    return async (url, options) => {
      ...
      if (url.endsWith("/api/v1/metrics/netrisk_lite")) {
        return jsonResponse(200, metricFixture("netrisk_lite"));
      }
  ...
  test("runtime mode resolution is exact and local only", () => {
    const cases = [
      ...
      [
        ...
        "backend-tools",
        "local-template",
        "backend_mode_requires_reviewed_deployment"
      ],
      [
      ...
      [
        "https://framerstoev.github.io/SPTC/coldwave-demo-v3/?assistantMode=backend-agent",
        "backend-agent",
        "local-template",
        "backend_mode_requires_reviewed_deployment"
      ],
  ...
  test("three methods construct exact reviewed requests", async () => {
    ...
    const networkMetric = await target.SPTCAssistant.client.explainMetric("netrisk_lite");
    ...
    equal(calls.map(call => call.url), [
      ...
      "http://127.0.0.1:8080/api/v1/metrics/netrisk_lite",
    ...
    assert(Object.isFrozen(metric) && Object.isFrozen(networkMetric) && Object.isFrozen(review));
  ...
  test("V3 assistant requests bind profile, tier, and authoritative active metric", async () => {
    const calls = [];
    const target = evaluateClient("http://localhost/page?assistantMode=backend-agent", {
      fetch: async (url, options) => {
        calls.push({ url, options });
    ...
    await target.SPTCAssistant.client.queryAssistant({
      message: "Explain the current Tier 2 network context.",
      selected_section_id: "CS_1081",
      active_metric: "netrisk_lite",
      active_analysis_layer: "tier2"
    });
    equal(JSON.parse(calls[0].options.body), {
      message: "Explain the current Tier 2 network context.",
      assistant_profile: "v3",
    ...
    await expectCode(
      () => target.SPTCAssistant.client.queryAssistant({
        ...
        active_analysis_layer: "event_rei"
      }),
      "invalid_request"
    );
    await expectCode(
      () => target.SPTCAssistant.client.queryAssistant({
        message: "Caller cannot select a profile.",
        assistant_profile: "v2"
      }),
      "invalid_request"
    );
    equal(calls.length, 1);
    ...
    for (const [intent, toolName, structuredResult] of fixtures) {
      const target = evaluateClient("http://localhost/page?assistantMode=backend-agent", {
        fetch: async () => jsonResponse(200, assistantFixture({
          ...
          warnings: [{
            section_id: null,
            code: intent === "summarize_tier_alignment"
              ? "METHOD_DEFINITION_REQUIRED"
              : "METHOD_SCOPE",
            severity: intent === "summarize_tier_alignment" ? "caution" : "info",
            message: "Reviewed descriptive scope applies."
          }],
  ...
  test("V3 structured results reject drift, inconsistent counts, and intent mismatches", async () => {
    const invalidPayloads = [];
    const wrongIntent = assistantFixture({
      intent: "explain_selected_section",
      structured_result: v3RankingFixture()
    });
    invalidPayloads.push(wrongIntent);
    const badDistribution = v3RankingFixture();
    badDistribution.distribution.missing_count += 1;
    invalidPayloads.push(assistantFixture({
      intent: "rank_sections",
      tools_used: [{ tool_name: "rank_sections", call_index: 1 }],
      structured_result: badDistribution
    }));
    const inventedAlignment = v3AlignmentFixture();
    inventedAlignment.consistent_count = 12;
    invalidPayloads.push(assistantFixture({
      intent: "summarize_tier_alignment",
      tools_used: [{ tool_name: "summarize_tier_alignment", call_index: 1 }],
      structured_result: inventedAlignment
    }));
    const wrongMetricSource = v3RankingFixture();
    wrongMetricSource.metric.source_metric = "weather_rei";
    invalidPayloads.push(assistantFixture({
      intent: "rank_sections",
      tools_used: [{ tool_name: "rank_sections", call_index: 1 }],
      structured_result: wrongMetricSource
    }));
    const wrongAlignmentRoles = v3AlignmentFixture();
    wrongAlignmentRoles.potential_metric = v3MetricDescriptor(
      "tier1_weather",
      "Tier 1 Weather",
      "weather_rei"
    );
    invalidPayloads.push(assistantFixture({
      intent: "summarize_tier_alignment",
      tools_used: [{ tool_name: "summarize_tier_alignment", call_index: 1 }],
      structured_result: wrongAlignmentRoles
    }));
    const missingCorrelations = v3AlignmentFixture();
    missingCorrelations.pearson_r = null;
    missingCorrelations.spearman_rho = null;
    invalidPayloads.push(assistantFixture({
      intent: "summarize_tier_alignment",
      tools_used: [{ tool_name: "summarize_tier_alignment", call_index: 1 }],
      structured_result: missingCorrelations
    }));
    invalidPayloads.push(assistantFixture({
      intent: "rank_sections",
      tools_used: [{ tool_name: "rank_sections", call_index: 1 }]
    }));
    invalidPayloads.push(assistantFixture({ structured_result: null }));
    for (const payload of invalidPayloads) {
      const target = evaluateClient("http://localhost/page?assistantMode=backend-agent", {
        fetch: async () => jsonResponse(200, payload)
      });
      await expectCode(
        () => target.SPTCAssistant.client.queryAssistant({ message: "Reviewed query." }),
        "invalid_response"
      );
    }
  });
  ...
  test("q_min and internal Tier 2 explanation contracts validate", async () => {
    const target = evaluateClient("http://localhost/page?assistantMode=backend-tools", {
      fetch: async url => {
        const name = url.endsWith("netrisk_lite") ? "netrisk_lite" : "q_min";
        return jsonResponse(200, metricFixture(name));
      }
    });
    const qMin = await target.SPTCAssistant.client.explainMetric("q_min");
    const network = await target.SPTCAssistant.client.explainMetric("netrisk_lite");
    equal(qMin.metric.name, "q_min");
    equal(network.metric.name, "netrisk_lite");
  ...
  try {
    ...
    for (const current of tests) {
      try {
        ...
        throw new Error(`${current.name}: ${String(error?.message || error)}`);
      }
      passed += 1;
    }
    document.body.dataset.status = "passed";
    resultNode.textContent = `${passed} Phase 4A V3 JavaScript contract tests passed.`;
  } catch (error) {
